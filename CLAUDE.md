...
**API routes** (JSON; CORS restricted to `neewoodygh.com`/`www` + localhost; disallowed origins get the canonical origin, never reflected): `POST /api/auth/login`; member `GET/PUT /api/me`, `POST/DELETE /api/me/photo` (raw JPEG ≤300KB → R2 `concierge/members/<phone>.jpg`, one per member, client compresses to 512px first), `GET /api/directory` (approved only, incl. phone); public `GET /api/media/members/<phone>.jpg` (cached, ETag); admin `GET/POST /api/admin/members`, `PUT /api/admin/members/:phone` (status/role/is_founder/profile/reset-PIN), `POST/DELETE /api/admin/members/:phone/photo`, `POST /api/admin/payments` (upserts on member+period), `GET /api/admin/payments?period=YYYY-MM`. Full table in `/concierge-api/README.md`.
