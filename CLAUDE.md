...
- **Lifetime founders vs monthly payments UI:** under the finalized model the 100 founders pay a one-time GHS 100 (lifetime), but the admin payments UI still models monthly dues — so a founder who has "paid" appears in "Show unpaid" every subsequent month. Recommended fix once owner decides: exclude `is_founder=1` from the monthly unpaid list + denominator (parallel to admins), and/or track the one-time founder payment separately. **Awaiting owner decision on how they want to reconcile founder payments.**
