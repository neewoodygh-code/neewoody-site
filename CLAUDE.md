...
**API routes** (JSON; CORS restricted to `neewoodygh.com`/`www` + localhost; disallowed origins get the canonical origin, never reflected): `POST /api/auth/login`; member `GET/PUT /api/me` (incl. self-set `skill_level` apprentice|carpenter|master + `years_experience`, migration 0003), `POST/DELETE /api/me/photo` (raw JPEG ≤300KB → R2 `concierge/members/<phone>.jpg`, one per member, client compresses to 512px first), `GET /api/directory` (approved only, incl. phone + skill fields); public `GET /api/media/members/<phone>.jpg` (cached, ETag); admin `GET/POST /api/admin/members`, `PUT /api/admin/members/:phone` (status/role/is_founder/profile/skill/reset-PIN), `DELETE /api/admin/members/:phone` (permanent — body must echo `{confirm:<phone>}`, self-delete blocked, cascades payments/cutlists/attempts/photo; admin UI adds two sanity prompts on top), `POST/DELETE /api/admin/members/:phone/photo`, `POST /api/admin/payments` (upserts on member+period), `GET /api/admin/payments?period=YYYY-MM`. Full table in `/concierge-api/README.md`.

**Frontend** (static, no framework, green/gold/cream system, mobile-first, all `noindex`, out of `sitemap.xml`/nav):
- Shared helper `js/concierge.js` (`window.Concierge`: API base — overridable via localStorage `nwd-api-override` for local dev — token storage, `api()` fetch, `requireSession`, phone/WhatsApp helpers, specialty labels + legacy fallback + `SPECIALTY_EXAMPLES` popover text, `ZONE_GROUPS`/`fillZoneSelect`, `compressImage`/`uploadPhoto`).
- `/concierge/login.html` (phone + PIN → directory), `/concierge/directory.html` (compact horizontal member cards — small photo left, details right — area/trade filters, tap/hover trade popovers, WhatsApp, edit-own-profile modal with photo upload + zone select; at ≥12 members opens with a daily-rotating 6-member "Featured today" strip + select-your-area prompt instead of the full list), `/concierge/admin.html` (vanilla JS — add-member with zone dropdown, members table with inline status/role/founder/reset-PIN/photo-upload, record + view payments; admin-gated via `/api/me`).
