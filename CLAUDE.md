...
- **Cloudflare D1** database `concierge` (NOT KV). Migrations: `/concierge-api/migrations/0001_initial.sql` — tables `members` (phone PK), `payments`, `login_attempts`. Specialties are a fixed vocabulary (**revised 2026-07-15; extended 2026-07-19**): `cabinet_construction, interior_work, solid_wood_furniture, upholstery, finishing_spray, outdoor_structures, site_construction, cnc_machining, interior_design, wall_paneling, lighting, deco_supply, other` (2026-07-19 added the last four for interior designers/vendors; was: furniture/glass_aluminium — legacy keys still render via fallback labels in `js/concierge.js`, rejected on save; `finishing_spray` kept deliberately — see CONCIERGE_CONTEXT.md). **Member type (`member_type`, migration 0010, 2026-07-19):** `carpenter` (default) | `interior_designer` | `vendor` — the primary identity, separate from `is_business`. Drives an at-a-glance icon badge on directory cards (green saw = carpenter, gold armchair = designer, bronze tag = vendor; `MEMBER_TYPE_*` + `memberTypeIcon/Color` in `js/concierge.js`) and whether the carpenter skill ladder (apprentice→master) shows — designers/vendors lead with their type instead. Self-set at register + in edit-profile, admin-settable in add/edit; directory has a type filter. The two existing vendors are re-typed by admin via the edit modal. Areas use a fixed zone vocabulary (`ZONE_GROUPS` in `js/concierge.js`: ~22 Greater Accra zones + all 15 other regions); column stays TEXT, constraint is frontend-only. Maps deferred (pin + Maps-app link later, no embedded tiles).
