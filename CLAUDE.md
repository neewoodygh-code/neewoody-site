...
- **Nadia Dome (2025):** Full home commission — four pieces to one brief: (1) wave-carved Guarea chest of drawers with continuous wave pattern across all drawer fronts, integrated recessed pulls, walnut-toned finish — hero piece, one of the most distinctive in the portfolio; (2) compact kitchenette in russet laminate with dark stone-effect countertop, undermount black sink, pull-out mixer tap, matte black bar handles; (3) three solid wood bar stools with dark velvet upholstered seats and X-cross stretcher base; (4) full-height two-door fitted wardrobe in dark laminate with three drawers below. All pieces share matte black hardware. Kitchen island and main kitchen cabinetry visible in some photos are pre-existing — not Neewoody's work, never claim them. Kitchenette videos (laundry room space): TAbjiBb5Bt0, 4EWuw0lJXQw, MzB3WHXUt9w, _wg_vHSfF-U. Wave chest video: KOX5r2eqUVE. Images in `images/projects/nadia-dome/`. Case study: `projects/nadia-dome.html`.
...
- **Achimota Guesthouse — Room Conversion (2026):** Full single-room upgrade to Airbnb standard for a guesthouse owner in Achimota. Scope spanned multiple categories in one commission: queen bed + nightstand, wardrobe, dresser, desk, wall paneling, wall molding, TV unit, glass display cabinets, and wainscotting in the adjoining hallway. **Hybrid placement** (no standalone case study page): the full multi-image project story lives on `portfolio.html` as a dedicated dark-green feature section (anchor `#achimota-guesthouse`, 8 images, lead `achimota-bed-wardrobe-wide`), with single representative cards on the relevant category pages all linking back to that anchor — `beds.html` (`achimota-bed-wardrobe-wide`), `wardrobes.html` (`achimota-wardrobe-vanity-wide`), and `tv-units.html` (an expanded TV + paneling feature section, anchor `#achimota`, 5 images). **Wall paneling/molding has no dedicated category page — by explicit direction it is grouped with TV Units**, since paneling and TV-unit work are typically ordered together. Images in `images/projects/achimota-guesthouse/` (WebP + JPG `<picture>` pairs, per the Image Format Policy; all 8 live only in this folder, referenced cross-page by path). Copy is placeholder pending final copy. Homepage not updated. Note for wardrobes: the new card is an `<a>` (`a.wd-card{display:block}`) with its image wrapper marked `.no-lb` so the gallery lightbox JS (`.wd-card-img:not(.no-lb)`) skips it and it navigates to the portfolio anchor instead.
